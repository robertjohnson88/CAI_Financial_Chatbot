...
if st.button("Search"):
    if query:
        ...
        if results:
            response = "\n".join([f"**Company:** {res['Company']}, **Year:** {res['Year']}\n**Market Cap:** {res['Market Cap(in B USD)']}B, **Revenue:** {res['Revenue']}, **Net Income:** {res['Net Income']}\n**EPS:** {res['Earning Per Share']}" for res in results])
        else:
